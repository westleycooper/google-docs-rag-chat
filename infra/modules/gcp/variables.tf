...
variable "name" {
  description = "Resource name prefix. Short: several GCP names are length-capped."
  type        = string
  default     = "ragoogle"
...
variable "postgres_tier" {
  description = "Cloud SQL machine tier. HNSW index builds are memory-hungry (ADR-0011)."
  type        = string
  default     = "db-custom-2-7680"
}

variable "postgres_storage_gb" {
  description = "Storage in GB. Vectors dominate: ~4KB per chunk at 1024 dims."
  type        = number
  default     = 64
}
...
variable "project_id" {
  description = "GCP project id. Required: unlike Azure and AWS, GCP has no ambient default."
  type        = string
}
