/**
 * Generated by orval v7.21.0 🍺
 * Do not edit manually.
 * Ragoogle API
 * 
RAG chat over Google Drive and other document sources.

**Retrieval** is hybrid: dense pgvector search and Postgres full-text search are
fused with Reciprocal Rank Fusion, then reranked (ADR-0004). Every turn streams
its retrieval trace so a wrong answer can be diagnosed rather than merely
noticed (ADR-0009).

**Permissions** are respected during ingestion. A folder that cannot be read is
skipped *with an audit record* and never fails the run, because a silent skip is
indistinguishable from an empty folder (ADR-0003).

 * OpenAPI spec version: 0.1.0
 */
import type { RetrievalScoreOutMrr } from './retrievalScoreOutMrr';
import type { RetrievalScoreOutNdcg } from './retrievalScoreOutNdcg';
import type { RetrievalScoreOutPrecision } from './retrievalScoreOutPrecision';
import type { RetrievalScoreOutRecall } from './retrievalScoreOutRecall';

export interface RetrievalScoreOut {
  expected_count: number;
  found_nothing: boolean;
  k: number;
  mrr: RetrievalScoreOutMrr;
  ndcg: RetrievalScoreOutNdcg;
  precision: RetrievalScoreOutPrecision;
  /** null when the case has no ground truth */
  recall: RetrievalScoreOutRecall;
  retrieved_count: number;
}
